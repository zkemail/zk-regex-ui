<script lang="js">
	...
	function generate() {
		...
		circom = genCircomAllstr(minDfa, 'Test', r, regex, inputs, false);
	}
	function generateZKReplCode() {
		let r = reveals.map((r1) => r1.map((r2) => r2.split(',')));
		circom = genCircomAllstr(minDfa, 'Test', r, regex, inputs, true);
	}
...
<div class="h-auto w-full text-primary-content mb-16 mt-32">
			...
			<form class="w-full">
				<div class="form-control w-full mb-4">
					...
					<input
						id="regex"
						type="text"
						class:input-error={!validateRegex(regex)}
						bind:value={regex}
				...
				<div class="form-control w-full mb-4">
					...
					<textarea
						id="input1"
						class:input-error={!validateInput(inputs[0], regex)}
						bind:value={inputs[0]}
						placeholder=""
				...
				<div class="form-control w-full mb-4">
					...
					<textarea
						id="input2"
						class:input-error={!validateInput(inputs[1], regex)}
						bind:value={inputs[1]}
						placeholder=""
						class="textarea textarea-bordered w-full"
					/>
				...
				<div class="form-control w-full mb-4">
					...
					<textarea
						id="input3"
						class:input-error={!validateInput(inputs[2], regex)}
						bind:value={inputs[2]}
						placeholder=""
						class="textarea textarea-bordered w-full"
					/>
			...
			{#if parsedInputs.length > 0}
				<div class="text-left w-full mt-4">
									...
									<thead>
										<tr class="border-b-stone-300">
											{#each states as state}
												<th
													class="p-0 w-5 cursor-pointer"
													on:click={() => {
														selectRevealState(state);
													}}>{state[1]}</th
												>
											{/each}
										</tr>
									...
									<tbody>
										<tr class="border-b-stone-300">
											{#each states as state}
												<td
													class="p-0 w-5 cursor-pointer"
													class:bg-yellow-50={stateSelected(state, reveals, currentReveal)}
													on:click={() => {
														selectRevealState(state);
													}}>{displayChar(state[2])}</td
												>
											{/each}
										</tr>
									</tbody>
					...
					<div class="mt-10 w-full">
						<div class="text-xl">Selected Reveal Groups</div>
						<div class="flex flex-col">
							{#each reveals as reveal, i}
								<div class="flex flex-row items-center">
									<button
										class="btn btn-sm btn-circle btn-ghost text-red-500"
										on:click={() => removeReveal(i)}>✕</button
									>
									<div
										class="mr-4 cursor-pointer"
										on:click={() => (currentReveal = i)}
										class:font-bold={currentReveal === i}
									>
										Reveal #{i}
									</div>
					...
					<button class="btn mt-4 w-full" on:click={generate} onclick="my_modal_2.showModal()"
						>Generate</button
					>
					<button
						class="btn mt-4 w-full"
						on:click={generateZKReplCode}
						onclick="my_modal_2.showModal()">Open in ZKRepl</button
					>
				</div>
			{/if}
		</div>
	...
	<dialog id="my_modal_2" class="modal">
		<div class="modal-box prose" style="min-width:800px;">
			...
			<pre>
        <svg
					class="shrink-0 h-5 w-5 transition text-gray-500 group-hover:text-white absolute right-10 cursor-pointer"
					xmlns="http://www.w3.org/2000/svg"
					viewBox="0 0 20 20"
					fill="currentColor"
					aria-hidden="true"
					on:click={() => navigator.clipboard.writeText(circom)}>
